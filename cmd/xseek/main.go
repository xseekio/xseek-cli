...
func main() {
	...
	case "products":
		if len(args) < 2 {
			fmt.Fprintln(os.Stderr, "Usage: xseek products <website> [--region <r>] [--category <c>] [--q <query>] [--limit <n>]")
			os.Exit(1)
		}
		commands.ListProducts(args[1], flags["region"], flags["category"], flags["q"], flags["limit"])
