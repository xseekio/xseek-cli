...
Commands:
  ...
  external-profiles <website>       Directories you already have a profile on
...
var commandHelp = map[string]string{
	...
	"external-profiles": "Usage: xseek external-profiles <website>\n\nList the directory/platform profiles the brand already maintains (G2, Capterra, …).\nUsed to avoid recommending a listing for a directory you're already on.\n\nFlags:\n  --format json    Output as JSON",
...
func main() {
	...
	case "external-profiles":
		if len(args) < 2 {
			fmt.Fprintln(os.Stderr, "Usage: xseek external-profiles <website>")
			os.Exit(1)
		}
		commands.ListExternalProfiles(args[1])
	...
	case "products":
		if len(args) < 2 {
			fmt.Fprintln(os.Stderr, "Usage: xseek products <website> [--<field> <value>]... [--q <query>] [--limit <n>]")
			os.Exit(1)
		}
		commands.ListProducts(args[1], flags)
	...
	case "articles":
		...
		case "push":
			...
			commands.PushArticle(args[2], flags["title"], flags["file"], flags["status"], flags["meta-description"], flags["keyword-term"], flags["keywords"], flags["opportunity-id"], flags["visuals"], flags["description"], flags["meta-title"], flags["claims"])
		case "get":
			if len(args) < 4 {
		...
		case "update":
			if len(args) < 4 {
				fmt.Fprintln(os.Stderr, "Usage: xseek articles update <website> <articleId> [--file article.md] [--title \"...\"] [--status draft] [--opportunity-id <uuid>|none] [--visuals <path.json>]")
				os.Exit(1)
			}
			commands.UpdateArticle(args[2], args[3], flags["file"], flags["title"], flags["status"], flags["meta-description"], flags["opportunity-id"], flags["visuals"], flags["description"], flags["meta-title"], flags["claims"])
		case "publish":
			if len(args) < 5 {
